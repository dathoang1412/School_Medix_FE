...
const VaccineCampaignSurvey = () => {
  ...
  const handleAccept = async () => {
    ...
    try {
      ...
      navigate(-1); // Quay lại trang trước mà không reload
    } catch (err) {
                ...
                {refuseReason.trim() && (
                  <div className="mt-3 flex justify-end">
                    <button
                      ...
                      disabled={processing}
                      className="bg-yellow-600 text-white px-6 py-2 rounded-lg hover:bg-yellow-700 disabled:bg-yellow-300 disabled:cursor-not-allowed transition-colors flex items-center space-x-2"
                    >
                      {processing ? (
                        <>
