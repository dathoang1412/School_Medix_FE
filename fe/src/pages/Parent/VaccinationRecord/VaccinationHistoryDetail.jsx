...
import { FileText, Calendar, MapPin, Pill, Loader2, XCircle } from 'lucide-react';
...
const VaccinationHistoryDetail = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            setError('');
            try {
                const response = await axiosClient.get(`/vaccination-record/${record_id}`);
                ...
                if (fetchedRecord.student_id !== student_id) {
                    ...
                    setLoading(false);
                    setTimeout(
                        () => navigate(`/parent/edit/${student_id}/vaccination-history`),
                        2000
                    );
                    return;
                }
    ...
    const handleBack = () => {
        navigate(`/parent/edit/${student_id}/vaccine-info`);
    };
            ...
            <div className="max-w-3xl mx-auto">
                ...
                <div className="bg-white shadow-md border border-gray-300 p-6 h-230">
                    <div className="flex items-center gap-3 mb-6">
                        <h1 className="text-2xl font-semibold text-gray-800">Hồ sơ tiêm chủng</h1>
                    </div>
                        ...
                        {userRole === 'parent' && (
                            <div className="space-y-4">
                                <div>
                                    ...
                                    <div className="space-y-4">
                                        <h5 className="text-sm font-bold text-gray-800">I. Thông tin tiêm</h5>
                                        {[
                                            { label: "Vaccine", value: record?.vaccine_name || 'Chưa xác định' },
                                            { label: "Bệnh", value: record?.disease_name || 'Chưa xác định' },
                                            { label: "Ngày tiêm", value: formatDate(record?.vaccination_date) },
                                        ].map(({ label, value }) => (
                                            <div key={label} className="flex items-start">
                                                <label className="w-1/4 text-sm font-bold text-gray-800">{label}</label>
                                                <p className="flex-1 text-sm text-gray-800">{value}</p>
                                    ...
                                    <div className="space-y-4 mt-6">
                                        <h5 className="text-sm font-bold text-gray-800">II. Thông tin bổ sung</h5>
                                        {[
                                            { label: "Địa điểm", value: record?.location || 'Không có' },
                                            { label: "Mô tả", value: record?.description || 'Không có' },
                                            { label: "Trạng thái", value: record?.status === 'COMPLETED' ? "Hoàn thành" : record?.status === 'PENDING' ? "Chờ xử lý" : "Đã hủy" || 'Không xác định' },
                                        ].map(({ label, value }) => (
                                            <div key={label} className="flex items-start">
                                                <label className="w-1/4 text-sm font-bold text-gray-800">{label}</label>
                                                <p className="flex-1 text-sm text-gray-800">{value}</p>
