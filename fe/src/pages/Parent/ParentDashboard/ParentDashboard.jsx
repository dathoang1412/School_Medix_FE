...
const ParentDashboardWrapper = () => {
          ...
          childrenData.map(async (child) => {
            try {
              const response = await axiosClient.get(
                `/dashboard/${child.id}/parent-dashboard`
              );
              return { ...child, sumNoti: response.data.data.sumNoti || 0 };
            } catch (err) {
              console.error(
                `Error fetching dashboard for child ${child.id}:`,
                err
              );
  ...
  useEffect(() => {
    const fetchDashboard = async () => {
      ...
      try {
        const response = await axiosClient.get(
          `/dashboard/${selectedChild.id}/parent-dashboard`
        );
        ...
        <div className="mb-8 flex justify-between items-center">
          <div>
            ...
            <p className="mt-2 text-gray-600">
              Quản lý thông tin y tế học đường cho con em
            </p>
          </div>
          <button
                ...
                {selectedChild ? (
                  ...
                  <div className="text-center py-12">
                    ...
                    <p className="mt-1 text-gray-500">
                      Vui lòng chọn một trẻ em từ danh sách bên trái để xem các
                      dịch vụ y tế
                    </p>
                  </div>
                )}
