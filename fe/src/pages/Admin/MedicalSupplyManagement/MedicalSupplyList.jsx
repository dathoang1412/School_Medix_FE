...
import { Loader2, Package, Edit, FileText, Syringe, Search, Calendar, ChevronDown, ChevronUp, Trash2 } from "lucide-react";
...
const MedicalSupplyList = () => {
  ...
  const confirmDelete = async () => {
    try {
      ...
      let errorMessage = "Có lỗi xảy ra khi xóa vật tư y tế.";
      if (err.message === "Không tìm thấy id vật tư y tế để xóa!" ||
          err.message === "Không tìm thấy để cập nhật") {
        errorMessage = "Không tìm thấy vật tư y tế để xóa.";
      } else if (err.message) {
  ...
  return (
    <div className="min-h-screen bg-white-50">
      ...
      <Modal
        ...
        cancelText="Hủy"
      >
        Bạn có chắc muốn xóa vật tư y tế "{itemToDelete?.name}"?
      </Modal>
    </div>
  );
};
